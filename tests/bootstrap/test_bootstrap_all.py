...
from algorun.core.bootstrap import ALGORUN_CONFIG
...
def test_bootstrap_all_algorun_min_version(tmp_path_factory: TempPathFactory) -> None:
    cwd = tmp_path_factory.mktemp("cwd")
    current_version = get_current_package_version()
    (cwd / ALGORUN_CONFIG).write_text('[algorun]\nmin_version = "999.99.99"\n')
    result = invoke(
        "bootstrap all",
        cwd=cwd,
    )
...
def test_bootstrap_all_algorun_min_version_ignore_error(tmp_path_factory: TempPathFactory) -> None:
    cwd = tmp_path_factory.mktemp("cwd")
    current_version = get_current_package_version()
    (cwd / ALGORUN_CONFIG).write_text('[algorun]\nmin_version = "999.99.99"\n')
    result = invoke(
