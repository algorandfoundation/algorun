...
def _mock_doctor_dependencies(mocker: MockerFixture) -> None:
    mocker.patch("algorun.cli.doctor.get_current_package_version").return_value = "1.2.3"
    mocker.patch("algorun.cli.doctor.get_latest_github_version").return_value = "1.2.3"
    # Mock datetime
    mocker.patch("algorun.cli.doctor.dt").datetime.now.side_effect = lambda _, tz=None: datetime(
        1990, 12, 31, 10, 9, 8, tzinfo=tz
    )
    # Mock shutil
    mocker.patch("algorun.core.doctor.which").side_effect = mock_shutil_which
    # Mock sys - Tuple[int, int, int, str, int]
    sys_module = mocker.patch("algorun.cli.doctor.sys")
    sys_module.version = "3.6.2"
    sys_module.prefix = "/home/me/.local/pipx/venvs/algorun"
...
def test_doctor_with_copy(mocker: MockerFixture) -> None:
    # Mock pyclip
    mocked_os = mocker.patch("algorun.cli.doctor.pyclip.copy")
...
def test_unexpected_exception_locating_executable(mocker: MockerFixture) -> None:
    ...
    mocker.patch("algorun.core.doctor.which").side_effect = which_throw
...
@pytest.mark.usefixtures("_mock_doctor_dependencies")
@pytest.mark.mock_platform_system("Darwin")
def test_new_algorun_version_available(request: pytest.FixtureRequest, mocker: MockerFixture) -> None:
    mocker.patch("algorun.cli.doctor.get_latest_github_version").return_value = "4.5.6"
    result = invoke("doctor")
