...
from algorun.core.conf import get_app_config_dir, get_app_state_dir, get_current_package_version

logger = logging.getLogger(__name__)

LATEST_URL = "https://api.github.com/repos/algorandfoundation/algorun/releases/latest"
...
def do_version_prompt() -> None:
    ...
    if _get_version_sequence(current_version) < _get_version_sequence(latest_version):
        logger.info(f"You are using algorun version {current_version}, however version {latest_version} is available.")
    else:
...
@click.command(
    "version-prompt", short_help="Enables or disables checking and prompting if a new version of algorun is available"
)
...
def version_prompt_configuration_command(*, enable: str | None) -> None:
    """Controls whether algorun checks and prompts for new versions.
    Set to [disable] to prevent algorun performing this check permanently, or [enable] to resume checking.
