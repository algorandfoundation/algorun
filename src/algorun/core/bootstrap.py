...
def bootstrap_any(project_dir: Path) -> None:
    ...
    if env_path.exists():
        logger.debug("Running `algorun bootstrap env`")
    ...
    if poetry_path.exists() or (pyproject_path.exists() and "[tool.poetry]" in pyproject_path.read_text("utf-8")):
        logger.debug("Running `algorun bootstrap poetry`")
        bootstrap_poetry(project_dir)

    if package_json_path.exists():
        logger.debug("Running `algorun bootstrap npm`")
    ...
    if try_install_poetry:
        ...
        if not questionary_extensions.prompt_confirm(
            ...
            raise click.ClickException(
                ...
                "manually via https://python-poetry.org/docs/ and try `algorun bootstrap poetry` again."
            )
    ...
    try:
        ...
        if try_install_poetry:
            raise click.ClickException(
                ...
                "and try `algorun bootstrap poetry` again."
            ) from e
...
def _find_valid_pipx_command() -> list[str]:
    ...
    #   this is an exceptional circumstance since pipx should always be present with algorun
    #   since it's installed with brew / choco as a dependency, and otherwise is used to install algorun
    raise click.ClickException(
        "Unable to find pipx install so that poetry can be installed; "
        "please install pipx via https://pypa.github.io/pipx/ "
        "and then try `algorun bootstrap poetry` again."
    )
...
def _get_base_python_path() -> str | None:
    ...
    if not this_python:
        ...
        return None
    # not in venv... not recommended to install algorun this way, but okay
...
def get_min_algorun_version(project_dir: Path) -> str | None:
    try:
        ...
        logger.debug(f"Unexpected error inspecting algorun config: {ex}")
        return None


def project_minimum_algorun_version_check(project_dir: Path, *, ignore_version_check_fail: bool = False) -> None:
    """
    Checks the current version of algorun against the minimum required version specified in the algorun config file.
    """

    min_version = get_min_algorun_version(project_dir)
    if min_version is None:
        return
    algorun_version = get_current_package_version()
    if version.parse(algorun_version) < version.parse(min_version):
        message = (
            f"This template requires algorun version {min_version} or higher, "
            f"but you have algorun version {algorun_version}. Please update algorun."
        )
